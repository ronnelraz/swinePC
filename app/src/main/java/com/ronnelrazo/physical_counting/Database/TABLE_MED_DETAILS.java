package com.ronnelrazo.physical_counting.Database;

public class TABLE_MED_DETAILS {

    public static final String TABLE_MED_DETAILS = "TABLE_MED_DETAILS";

    public String POSITION = "POSITION"; //PK
    public String ORG_CODE = "ORG_CODE"; //fk
    public String BUCODE = "BUCODE";
    public String BU_TYPE_CODE = "BU_TYPE_CODE";
    public String FARM_CODE = "FARM_CODE"; //fk;
    public String FARM_ORG = "FARM_ORG";
    public String FARM_NAME = "FARM_NAME";
    public String MED_CODE = "MED_CODE";
    public String MED_NAME = "MED_NAME";
    public String SYS_MED_STOCK_QTY = "SYS_MED_STOCK_QTY";
    public String SYS_MED_STOCK_WGH = "SYS_MED_STOCK_WGH";
    public String STOCK_UNIT = "STOCK_UNIT";
    public String COUNTING_STOCK = "COUNTING_STOCK";
    public String REMARK = "REMARK";
    public String VARIANCE = "VARIANCE";
    public String UNPOST = "UNPOST";
    public String ACTIVE_VAR = "ACTIVE_VAR";












}
