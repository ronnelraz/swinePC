...
public class BreederTab_adapter_edit extends RelativeLayout {
    ...
    Context context;
    String str_auditno;
    public List<model_breeder_edit> dana = new ArrayList<>();

    int headerCellsWidth[] = new int[headers.length];

    public BreederTab_adapter_edit(Context context, String str_auditno) {
        super(context);
    ...
    private void putangina(){
        ...
        API.getClient().edit_breeder(str_auditno).enqueue(new Callback<Object>() {
            @Override
                ...
                try {
                    ...
                    if(success){
                        for (int i = 0; i < result.length(); i++) {
                            ...
                            model_breeder_edit sampleObject = new model_breeder_edit(
...
//        this.horizontalScrollViewB.setBackgroundColor(Color.LTGRAY);
    ...
    private void generateTableC_AndTable_B(){
        ...
        for(int i = 0; i < dana.size(); i++){
            ...
            final model_breeder_edit getData = dana.get(i);
    ...
    TableRow tableRowForTableC(model_breeder_edit sampleObject,int i,int position){
...
//        textView.setBackgroundColor(Color.parseColor("#FFFFFF"));
    ...
    TableRow taleRowForTableD(model_breeder_edit sampleObject,int position){
        final model_breeder_edit getData = dana.get(position);
        int backgroundColor = position%2==0 ? R.color.even : R.color.odd;
        ...
        String info[] = {
                ...
                sampleObject.getVarience(),
                sampleObject.getUnpost(),
                sampleObject.getActive_var(),
                        ...
                        if(s.toString().matches(dash)){
                            ...
                            if (s.length() > 0) {
                                try{
                                    ...
                                    updateChanges(count,getRemarks,getData.getOrg_Code(),str_auditno,getData.getFarm_Org(),String.valueOf(varience_total),getunpost,getactiveVaR);
                        ...
                        if(s.toString().matches(dash)){
                            ...
                            if (s.length() > 0) {
                                try{
                                    ...
                                    updateChanges(count,getRemarks,getData.getOrg_Code(),str_auditno,getData.getFarm_Org(),getvariance,getunpost,getactiveVaR);
    ...
    protected void updateChanges(String counting,String remark,String org_code,String audit_no,String farm_org,String variance,String unpost,String actual){

        Log.d("putangina","\n"+counting + "\n" + remark+ "\n" +org_code+ "\n" +audit_no+ "\n" +farm_org+ "\n" +variance+ "\n" +unpost+ "\n" +actual);

        API.getClient().update_breeder_idana(counting,remark,org_code,audit_no,farm_org,variance,unpost,actual, SharedPref.getInstance(context).getUser()).enqueue(new Callback<Object>() {
            @Override
            public void onResponse(Call<Object> call, retrofit2.Response<Object> response) {
                try {
