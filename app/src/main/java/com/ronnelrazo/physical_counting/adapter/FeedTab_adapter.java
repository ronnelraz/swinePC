...
public class FeedTab_adapter extends RelativeLayout {
    ...
    String str_farmOrg;
    String cutoffDate;
    public List<model_feed> dana = new ArrayList<>();

    int headerCellsWidth[] = new int[headers.length];

    public FeedTab_adapter(Context context, String str_orgCode, String str_farmOrg,String cutoffDate) {
        super(context);
    ...
    private void putangina(){
        API_.getClient().Feed(str_orgCode,str_farmOrg,Globalfunction.getInstance(context).getMonth(),cutoffDate).enqueue(new Callback<Object>() {
            @Override
...
//        this.horizontalScrollViewB.setBackgroundColor(Color.LTGRAY);
    ...
    private void generateTableC_AndTable_B(){
        ...
        for(int i = 0; i < dana.size(); i++){
            ...
            final model_feed getData = dana.get(i);
            ...
            boolean savefeed = Globalfunction.getInstance(context)
                    .ADD_FEED_DETAILS(i,
                            getData.getOrgCode(),
                            tab_from.bu_code,
    ...
    TableRow tableRowForTableC(model_feed sampleObject,int i,int position){
...
//        textView.setBackgroundColor(Color.parseColor("#FFFFFF"));
    ...
    TableRow taleRowForTableD(model_feed sampleObject,int position){
        final model_feed getData = dana.get(position);
        int backgroundColor = position%2==0 ? R.color.even : R.color.odd;
                        ...
                        if(s.toString().matches(dash)){
                            ...
                            if (s.length() > 0) {
                                try{
                                    ...
                                    boolean update = Globalfunction.getInstance(context)
                                            .updateFeedlist(position,getData.getOrgCode(),getData.getFarmCode(),count,getRemarks,String.valueOf(varience_total),getunpost,getactiveVaR);
                                    if(update){
                                        Log.d("cebu_feed","feed updated postion : " + position);
                                    }
                                    else{
                                        Log.d("cebu_feed","feed error postion : " + position);
                                    }


                                } catch(NumberFormatException ex){
                                    Log.d("cebu_feed","error number");
                                }
                        ...
                        if(s.toString().matches(dash)){
                            ...
                            if (s.length() > 0) {
                                try{
                                    ...
                                    boolean update = Globalfunction.getInstance(context)
                                            .updateFeedlist(position,getData.getOrgCode(),getData.getFarmCode(),count,getRemarks,getvariance,getunpost,getactiveVaR);
                                    if(update){
                                        Log.d("cebu_feed","feed updated postion : " + position);
                                    }
                                    else{
