...
public class MedTab_adapter extends RelativeLayout {
                ...
                try {
                    ...
                    if(success){
                        for (int i = 0; i < result.length(); i++) {
                            ...
                            model_med sampleObject = new model_med(
...
//        this.horizontalScrollViewB.setBackgroundColor(Color.LTGRAY);
    ...
    private void generateTableC_AndTable_B(){
        ...
        for(int i = 0; i < dana.size(); i++){
            ...
            final model_med getData = dana.get(i);
            ...
            boolean saveMed = Globalfunction.getInstance(context)
                    .ADD_MED_DETAILS(i,
                            getData.getOrgCode(),
                            tab_from.bu_code,
                            ...
                            "0");
            if(saveMed){
                Log.d("swine","med save : " + i);
            }
            else{
                Log.d("swine", "med error : " + i);
            }
        }
    ...
    TableRow tableRowForTableC(model_med sampleObject,int i,int position){
...
//        textView.setBackgroundColor(Color.parseColor("#FFFFFF"));
    ...
    TableRow taleRowForTableD(model_med sampleObject,int position){
        final model_med getData = dana.get(position);
        int backgroundColor = position%2==0 ? R.color.even : R.color.odd;
                            ...
                            if (s.length() > 0) {
                                try{
                                    ...
                                    boolean update = Globalfunction.getInstance(context)
                                            .updateMedlist(position,getData.getOrgCode(),getData.getFarmCode(),count,getRemarks,String.valueOf(varience_total),getunpost,getactiveVaR);
                                    if(update){
                        ...
                        if(s.toString().matches(dash)){
                            ...
                            if (s.length() > 0) {
                                try{
                                    ...
                                    boolean update = Globalfunction.getInstance(context)
                                            .updateMedlist(position,getData.getOrgCode(),getData.getFarmCode(),count,getRemarks,getvariance,getunpost,getactiveVaR);
                                    if(update){
                                        Log.d("cebu_feed","feed updated postion : " + position);
                                    }
                                    else{
