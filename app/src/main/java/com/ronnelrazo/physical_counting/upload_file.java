...
public class upload_file extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter = new Adapter_upload_list(list,this,intent,modal_interface);
    ...
    private void AutoCompleteCode(String user) {
       ...
       modal_interface = new modal_interface() {
           @Override
           public void onClick(View v, String audit, String type, List<model_viewfile> list, RecyclerView.Adapter adapte) {
               if(type.equals("img")){
                   adapterfunc_a(v,audit);
                   auditno = audit;
                   ifupdate = true;
               }else{
                   adapterfunc_b(v,audit);
                   auditno = audit;
                   ifupdate = true;
               }
           }
            ...
            public void onResponse(Call<Object> call, retrofit2.Response<Object> response) {
                try {
                    ...
                    if(success){
                        ...
                        adapter = new Adapter_upload_list(list,getApplicationContext(),intent,modal_interface);
                        recyclerView.setAdapter(adapter);
                    }
                    else{
                        list.clear();
...
//            Toast.makeText(this, getaudit, Toast.LENGTH_SHORT).show();
                    ...
                    if(d != null){
                        if(d.getClipData() != null){
                            for (int i = 0; i < d.getClipData().getItemCount(); i++) {
                                ...
                                if(!(i + 1 < d.getClipData().getItemCount())) {
                                    UploadPDFFile(getaudit,pdfName,pdfToString,"last",ConfirmDialog);
                                }
                                else{
                                    UploadPDFFile(getaudit,pdfName,pdfToString,"more",ConfirmDialog);
                                }
    ...
    public void UploadPDFFile(String audit,String name,String file,String last,AlertDialog dialog){
//        Toast.makeText(this, audit + " " + name + " " + Attach_Type,  Toast.LENGTH_SHORT).show();
